...
*The negative binomial model predicted that there is a 28% increase in lobster abundance at the treatment sites which is the same as what the poisson model predicted, while the simple OLS model only estimated a 5.9% increase.*
...
*For all three models, there was a consistent and statistically significant positive treatment affect. Both the negative binomial model and the poisson model predicted that there is a 28% increase in lobster abundance at the treatment sites, while the simple OLS model only estimated a 5.9% increase. The poisson and negative binomial models performed similarly, despite the presence of overdispersion in the poisson model, which indicates that the positive treatment effect is robust.*
...
ff_counts <- ff_counts |>
    group_by(year, mpa) |>
                 # group by year and mpa to calculate mean_count
                 summarise(mean_count = mean(counts, na.rm = TRUE)) |>
                 # convert year to factor
                 mutate(year = as.factor(year)) |>
     ungroup()

# Use interact_plot to plot mean predictions by year and treatment site
