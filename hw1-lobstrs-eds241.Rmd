...
#treat_group <- spiny_counts |> select(mean_size)
...
# linear model of treatment impact on counts
m1_ols <- lm(
    counts ~ treat,
    data = spiny_counts
)
...
In OLS there is an assumption of normality of residuals. In the case of the the qq plot, ideally we would like to see the points fall along the line, since we are seeing a curve it means that our data is violating the assumption of normality. Additionally, in the normality plot the distribution of our data does not fall along the normal curve.
...
exp(0.28) - 1
```
There is a 32% increase in lobster abundance at the treatment sites.
